Describe "Export-AxModelStoreV2 Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Export-AxModelStoreV2).ParameterSets.Name | Should -Be 'Generate'
		}
		
		It 'Should have the expected parameter DatabaseServer' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['DatabaseServer']
		...
		It 'Should have the expected parameter ModelstoreDatabase' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['ModelstoreDatabase']
		...
		It 'Should have the expected parameter InstanceName' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['InstanceName']
		...
		It 'Should have the expected parameter Suffix' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['Suffix']
		...
		It 'Should have the expected parameter Path' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['Path']
		...
		It 'Should have the expected parameter ShowOriginalProgress' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['ShowOriginalProgress']
		...
		It 'Should have the expected parameter OutputCommandOnly' {
			$parameter = (Get-Command Export-AxModelStoreV2).Parameters['OutputCommandOnly']
	...
	Describe "Testing parameterset Generate" {
		<#
		Generate -
		Generate -DatabaseServer -ModelstoreDatabase -InstanceName -Suffix -Path -ShowOriginalProgress -OutputCommandOnly
		#>
	}
