Describe "Set-AxLogicAppConfig Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Set-AxLogicAppConfig).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Url' {
			$parameter = (Get-Command Set-AxLogicAppConfig).Parameters['Url']
		...
		It 'Should have the expected parameter Email' {
			$parameter = (Get-Command Set-AxLogicAppConfig).Parameters['Email']
		...
		It 'Should have the expected parameter Subject' {
			$parameter = (Get-Command Set-AxLogicAppConfig).Parameters['Subject']
		...
		It 'Should have the expected parameter ConfigStorageLocation' {
			$parameter = (Get-Command Set-AxLogicAppConfig).Parameters['ConfigStorageLocation']
		...
		It 'Should have the expected parameter Temporary' {
			$parameter = (Get-Command Set-AxLogicAppConfig).Parameters['Temporary']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Url -Email -Subject
