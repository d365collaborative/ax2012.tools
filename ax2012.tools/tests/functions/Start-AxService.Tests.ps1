Describe "Start-AxService Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Start-AxService).ParameterSets.Name | Should -Be 'Default', 'Pipeline'
		}
		
		It 'Should have the expected parameter Server' {
			$parameter = (Get-Command Start-AxService).Parameters['Server']
		...
		It 'Should have the expected parameter DisplayName' {
			$parameter = (Get-Command Start-AxService).Parameters['DisplayName']
		...
		It 'Should have the expected parameter Name' {
			$parameter = (Get-Command Start-AxService).Parameters['Name']
		...
		It 'Should have the expected parameter ShowOriginalProgress' {
			$parameter = (Get-Command Start-AxService).Parameters['ShowOriginalProgress']
