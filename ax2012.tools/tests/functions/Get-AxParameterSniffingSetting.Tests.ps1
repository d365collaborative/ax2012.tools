Describe "Get-AxParameterSniffingSetting Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-AxParameterSniffingSetting).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter DatabaseServer' {
			$parameter = (Get-Command Get-AxParameterSniffingSetting).Parameters['DatabaseServer']
		...
		It 'Should have the expected parameter DatabaseName' {
			$parameter = (Get-Command Get-AxParameterSniffingSetting).Parameters['DatabaseName']
		...
		It 'Should have the expected parameter SqlUser' {
			$parameter = (Get-Command Get-AxParameterSniffingSetting).Parameters['SqlUser']
		...
		It 'Should have the expected parameter SqlPwd' {
			$parameter = (Get-Command Get-AxParameterSniffingSetting).Parameters['SqlPwd']
