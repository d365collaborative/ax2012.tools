Describe "Get-AxServerXppILHash Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-AxServerXppILHash).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter InstanceName' {
			$parameter = (Get-Command Get-AxServerXppILHash).Parameters['InstanceName']
		...
		It 'Should have the expected parameter OutputPath' {
			$parameter = (Get-Command Get-AxServerXppILHash).Parameters['OutputPath']
		...
		It 'Should have the expected parameter FileName' {
			$parameter = (Get-Command Get-AxServerXppILHash).Parameters['FileName']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -
		__AllParameterSets -InstanceName -OutputPath -FileName
		#>
	}
