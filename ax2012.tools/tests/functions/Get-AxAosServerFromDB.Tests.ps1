Describe "Get-AxAosServerFromDB Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-AxAosServerFromDB).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter DatabaseServer' {
			$parameter = (Get-Command Get-AxAosServerFromDB).Parameters['DatabaseServer']
		...
		It 'Should have the expected parameter DatabaseName' {
			$parameter = (Get-Command Get-AxAosServerFromDB).Parameters['DatabaseName']
		...
		It 'Should have the expected parameter SqlUser' {
			$parameter = (Get-Command Get-AxAosServerFromDB).Parameters['SqlUser']
		...
		It 'Should have the expected parameter SqlPwd' {
			$parameter = (Get-Command Get-AxAosServerFromDB).Parameters['SqlPwd']
